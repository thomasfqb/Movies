...
class TopRatedController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    fileprivate func fetchMovies() {
        MovieService.shared().fetchMovies(for: .topRated, page: nextPage) { (movies, nextPage, error) in
            if let error = error {
                print("Failed to fetch top ratedMovie" + error.localizedDescription)
            }
